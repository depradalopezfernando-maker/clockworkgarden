# Feasibility Analysis — Building *Clockwork Garden* with Claude Code
...
*Clockwork Garden* is close to a best-case project for Claude Code. It is a
...
*not* yet know (§10). That is exactly the input that makes agentic implementation
...
1. **Presentation.** Claude cannot judge whether a game *feels* good. Juice, game
...
2. **Balance beyond what a sim can prove.** A simulation can prove that Season 3
   takes 2.1 hours. It cannot prove that Season 3 is *fun*.
...
Scored for *agentic* implementation difficulty, which is not the same as human
...
| Insight tree (45–55 nodes) | §3 | **Medium** | Logic is simple; *content authoring* is the bulk, and the tree UI is fiddly |
...
| Balance tuning to 6–10 hrs | §8, §10 | **Medium** | *Simulation* is easy and Claude excels at it; *judgement* is not |
...
- **Visual verification is possible in this environment.** Chromium and Playwright
  are pre-installed. Claude can drive the running game, screenshot it, and *look at
  the image*. This is a genuine step up from writing UI blind — it catches layout
...
constants and can tell you the *consequences* of a value. It cannot tell you which
...
`palette.ts` with four Season ramps) *before* any asset work, so asset integration
...
*Isometric Miniature Farm* and *Nature Kit* packs recommended in §11 **cannot be
